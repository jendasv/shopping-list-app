...
class ItemAccessTest extends TestCase
{
    ...
    public function test_user_cannot_add_item_to_another_users_list(): void
    {
        ...
        $response = $this->actingAs($other)->postJson("/api/lists/{$list->id}/item", [
            'name' => 'Milk',
            'quantity' => 1,
        ]);

        $response->assertStatus(404);
        $this->assertDatabaseMissing('item', ['shopping_list_id' => $list->id]);
    }
    ...
    public function test_user_cannot_update_item_on_another_users_list(): void
    {
        ...
        $response->assertStatus(404);
        $this->assertDatabaseMissing('item', ['id' => $item->id, 'name' => 'Hacked']);
    }
    ...
    public function test_household_member_can_add_item_to_shared_list(): void
    {
        ...
        $response = $this->actingAs($member)->postJson("/api/lists/{$list->id}/item", [
            'name' => 'Butter',
            'quantity' => 1,
        ]);

        $response->assertStatus(201);
        $this->assertDatabaseHas('item', ['name' => 'Butter', 'shopping_list_id' => $list->id]);
    }
