        ...
        return $schema->components([
            ...
            Section::make(fn () => __('Statistics'))->schema([
                TextEntry::make('private_lists')
                    ...
                    ->getStateUsing(fn ($record) => $record->lists()->where('visibility', 'private')->count()),
                TextEntry::make('shared_lists')
                    ->label(fn () => __('Shared lists'))
                    ->getStateUsing(fn ($record) => $record->lists()->where('visibility', 'shared')->count()),
                TextEntry::make('visible_items')
                    ->label(fn () => __('Visible items'))
                    ->getStateUsing(function ($record) {
                        $ownListIds = $record->lists()->pluck('id');
                        ...
                        return ListItem::whereIn('list_id', $allIds)->count();
                    }),
            ])->columns(3),
        ]);
    }
